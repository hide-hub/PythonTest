
import numpy as np
import scipy.spatial.distance as ssd
import matplotlib.pyplot as plt
from scipy.cluster.hierarchy import dendrogram, linkage

# our genetic code
code = [ 'A', 'T', 'C', 'G' ]

# convert list of integers to corresponding letters
def to_code( a ) :
    return [ code[ii] for ii in a ]

# distance between 2 DNA strands
def dist(a, b) :
    return sum([ ii!=jj for ii, jj in zip(a,b) ])

# generate offspring by modifying some charactors in the code
def generate_offspring(parent) :
    return [maybe_modify(c) for c in parent]

# modify letter with probability ~1/1000
def maybe_modify(c) :
    if np.random.random() < 0.001 :
        return np.random.choice(code)
    return c

# create 3 distinct ancestors
p1 = to_code( np.random.randint(4, size=1000) )
p2 = to_code( np.random.randint(4, size=1000) )
p3 = to_code( np.random.randint(4, size=1000) )

# create offsprings
num_generations = 99
max_offspring_per_generation = 1000
current_generation = [p1, p2, p3]
for ii in range( num_generations ) :
    next_generation = []
    for parent in current_generation :
        # each parent will have between 1 and 3 children
        num_offspring = np.random.randint(3) + 1

        # generate the offspring
        for _ in range( num_offspring ) :
            child = generate_offspring( parent )
            next_generation.append( child )
        
    current_generation = next_generation

    # limit the number of offspring
    np.random.shuffle( current_generation )
    current_generation = current_generation[:max_offspring_per_generation]

    print( "Finished croning generation {} / {}, size {}".format(
        ii + 2,
        num_generations + 1,
        len( current_generation )
    ) )

# create distance matrix
# note: same distance matrix will be returned by scipy's pdist function
#       but scilean's pairwise_distance is not
#       scilean's pairwise_distance is not symmetric matrix
N = len( current_generation )
dist_matrix = np.zeros( (N,N) )
for ii in range( N ) :
    for jj in range( N ) :
        if ii == jj :
            # diagonal values should be zero
            continue
        elif jj > ii :
            # upper triangle values are distance
            a = current_generation[ii]
            b = current_generation[jj]
            dist_matrix[ii, jj] = dist( a, b )
        else :
            # of course the lower triangle values are same with upper corresponding value
            dist_matrix[ii, jj] = dist_matrix[jj, ii]

dist_array = ssd.squareform( dist_matrix )

Z = linkage( dist_array, 'ward' )
plt.title( 'Ward' )
dendrogram( Z )
plt.show()

Z = linkage( dist_array, 'single' )
plt.title( 'Single' )
dendrogram( Z )
plt.show()

Z = linkage( dist_array, 'complete' )
plt.title( 'Complete' )
dendrogram( Z )
plt.show()

